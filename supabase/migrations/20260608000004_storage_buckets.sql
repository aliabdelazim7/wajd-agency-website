...
create policy "Authenticated Upload Access" on storage.objects 
  for insert to authenticated with check (bucket_id in ('portfolio_images', 'site_assets'));

create policy "Authenticated Delete Access" on storage.objects 
  for delete to authenticated using (bucket_id in ('portfolio_images', 'site_assets'));
